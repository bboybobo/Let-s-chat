...
public class GuiUtils {
	...
	static {
		...
		mainDeviceBounds = GraphicsEnvironment.getLocalGraphicsEnvironment()
	...
	public static Color getLookAndFeelColor(String colorName) {
		Color c = (Color) lookAndFeelColorRegister
				.get(colorName != null ? colorName.trim() : " ");
		// if(c == null){
		// c = Color.LIGHT_GRAY;
		// }
		return c;
	...
	public static int getComponentWidthForResolution(int screenWidth,
			int desiredWidth) {
		int currentScreenWidth = mainDeviceBounds.width > MAX_COMPONENTS_WIDTH ? MAX_COMPONENTS_WIDTH
				: mainDeviceBounds.width;
	...
	private static void setComponentOrientation() {
		...
		// } else {
		// if
		// ("ug".equalsIgnoreCase(Kernel.getInstance().state.getLocale().getLocale().getLanguage()))
		// {
		// componentOrientation = ComponentOrientation.RIGHT_TO_LEFT;
		// } else {
		// componentOrientation =
		// ComponentOrientation.getOrientation(Kernel.getInstance().state.getLocale().getLocale());
		// }
		//
		// }
	}
